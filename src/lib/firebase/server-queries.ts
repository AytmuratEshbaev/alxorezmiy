...
// Each read is wrapped in React.cache() so generateMetadata + page render dedupe within a
// single request (same args → one Firestore read).
export const getNewsList = cache(async (limit = 50): Promise<News[]> => {
...
export const getNewsById = cache(async (id: string): Promise<News | null> => {
...
export const getNewsIds = cache(async (): Promise<string[]> => {
  const snap = await adminDb.collection('news').where('status', '==', 'published').select().get();
...
export const getOlympiad = cache(async (): Promise<OlympiadResult[]> => {
